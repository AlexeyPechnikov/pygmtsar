...
main(int argc, char *argv[])
{
	...
	if (vm.count("version")) {
		std::cout << "make_slc_rs2 " << PACKAGE_VERSION << "\n"
	...
	RS2 rs2 = RS2(filename);
	std::cout << "Extracting Image" << std::endl;
	SingleBandImage<std::complex<short> > *image = rs2.extractSlc_short_Image(outputImage);
	if (image == NULL)
	{
		return 1;
	}

	GMTSAR::writeEphemeris(outputOrbit,rs2.getOrbit(),rs2.getScene());
	GMTSAR::writeResourceFile(outputRSC,rs2.getScene(),rs2.getInstrument(),rs2.getOrbit(),image);
