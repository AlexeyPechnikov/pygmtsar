 ...
 */
template <typename T>
class BSQImage : public MultiBandImage
{
public:
  T getValue(int x, int y, int band)
  {
    ...
    return this->image[this->width*y + x + band*this->width*this->height];
  }
  ...
  void setValue(int x, int y, int band, T val)
  {
    this->testBand(band);
    this->testCoordinates(x,y);
    this->image[this->width*y + x + band*this->width*this->height] = val;
  }
};
