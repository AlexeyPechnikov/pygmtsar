...
int main(int argc, char **argv){
    ...
    if ((stemin = fopen(argv[1],"r")) == NULL) die("Couldn't open stem file: \n",argv[1]);
    ...
    strcat(tmp_str,".PRM");
    if ((PRM = fopen(tmp_str,"r")) == NULL) die("Couldn't open prm file: \n",tmp_str);
    ...
    /* malloc buf for copying, make some extra space in case the width of two images are different */
    buf = (short *)malloc(width*2*sizeof(short)*2);

    /* open the output SLC file */
    strcpy(tmp_str,argv[2]);
    strcat(tmp_str,".SLC");
    if ((SLCout = fopen(tmp_str,"wb")) == NULL) die("Couldn't open output SLC file: \n",tmp_str);
    ...
    strcpy(tmp_str,argv[2]);
    strcat(tmp_str,".PRM");
    if ((PRM = fopen(tmp_str,"w")) == NULL) die("Couldn't open output PRM file: \n",tmp_str);
