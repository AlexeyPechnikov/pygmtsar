...
char *USAGE = " ALOS_mosaic_ss_2frames prmfile1 grd1 prmfile2 grd2 prmfile3 grd3 prmfile4 grd4 prmfile5 grd5 prmfile6 grd6 prmfile7 grd7 prmfile8 grd8 prmfile9 grd9 prmfile10 grd10 mosaic dec dir \n\n" 
" USAGE:  \n"
...
int main(int argc, char **argv) {
  ...
  struct GMT_GRID *grid1 = NULL, *grid2 = NULL, *grid3 = NULL, *grid4 = NULL, *grid5 = NULL, *grid6 = NULL, *grid7 = NULL, *grid8 = NULL, *grid9 = NULL, *grid10 = NULL, *grid11 = NULL; 
  double inc[2], wesn[4];
  ...
  if (dir == 1) {
    ...
    /* subswath 6 */
    if ((grid6 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[12], NULL)) == NULL) return EXIT_FAILURE;
    /* check if the dimension of the grid and PRM file match */
    fprintf(stderr,"subswath 6: num_rng=%d, num_azi=%d\n",num_rng[5],num_azi[5]);
    fprintf(stderr,"gmt grid nx=%d, gmt grid inc_x=%f, gmt grid ny=%d, gmt grid inc_y=%f\n",grid6->header->nx,grid6->header->inc[GMT_X],grid6->header->ny,grid6->header->inc[GMT_Y]);
    if ((num_rng[5] != round(grid6->header->nx*grid6->header->inc[GMT_X])) || (num_azi[5] != round(grid6->header->ny * grid6->header->inc[GMT_Y]))) {
      die("\nPRM file and GMT grid don't match!\n",argv[12]);
    }
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[12], grid6) == NULL) return EXIT_FAILURE;

    /* subswath 7 */
    if ((grid7 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[14], NULL)) == NULL) return EXIT_FAILURE;
    /* check if the dimension of the grid and PRM file match */
    fprintf(stderr,"subswath 7: num_rng=%d, num_azi=%d\n",num_rng[6],num_azi[6]);
    fprintf(stderr,"gmt grid nx=%d, gmt grid inc_x=%f, gmt grid ny=%d, gmt grid inc_y=%f\n",grid7->header->nx,grid7->header->inc[GMT_X],grid7->header->ny,grid7->header->inc[GMT_Y]);
    if ((num_rng[6] != round(grid7->header->nx*grid7->header->inc[GMT_X])) || (num_azi[6] != round(grid7->header->ny * grid7->header->inc[GMT_Y]))) {
      die("\nPRM file and GMT grid don't match!\n",argv[14]);
    }
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[14], grid7) == NULL) return EXIT_FAILURE;

    /* subswath 8 */
    if ((grid8 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[16], NULL)) == NULL) return EXIT_FAILURE;
    /* check if the dimension of the grid and PRM file match */
    fprintf(stderr,"subswath 8: num_rng=%d, num_azi=%d\n",num_rng[7],num_azi[7]);
    fprintf(stderr,"gmt grid nx=%d, gmt grid inc_x=%f, gmt grid ny=%d, gmt grid inc_y=%f\n",grid8->header->nx,grid8->header->inc[GMT_X],grid8->header->ny,grid8->header->inc[GMT_Y]);
    if ((num_rng[7] != round(grid8->header->nx*grid8->header->inc[GMT_X])) || (num_azi[7] != round(grid8->header->ny * grid8->header->inc[GMT_Y]))) {
      die("\nPRM file and GMT grid don't match!\n",argv[16]);
    }
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[16], grid8) == NULL) return EXIT_FAILURE;

    /* subswath 9 */
    if ((grid9 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[18], NULL)) == NULL) return EXIT_FAILURE;
    /* check if the dimension of the grid and PRM file match */
    fprintf(stderr,"subswath 9: num_rng=%d, num_azi=%d\n",num_rng[8],num_azi[8]);
    fprintf(stderr,"gmt grid nx=%d, gmt grid inc_x=%f, gmt grid ny=%d, gmt grid inc_y=%f\n",grid9->header->nx,grid9->header->inc[GMT_X],grid9->header->ny,grid9->header->inc[GMT_Y]);
    if ((num_rng[8] != round(grid9->header->nx*grid9->header->inc[GMT_X])) || (num_azi[8] != round(grid9->header->ny * grid9->header->inc[GMT_Y]))) {
      die("\nPRM file and GMT grid don't match!\n",argv[18]);
    }
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[18], grid9) == NULL) return EXIT_FAILURE;

    /* subswath 10 */
    if ((grid10 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[20], NULL)) == NULL) return EXIT_FAILURE;
    /* check if the dimension of the grid and PRM file match */
    fprintf(stderr,"subswath 10: num_rng=%d, num_azi=%d\n",num_rng[9],num_azi[9]);
    fprintf(stderr,"gmt grid nx=%d, gmt grid inc_x=%f, gmt grid ny=%d, gmt grid inc_y=%f\n",grid10->header->nx,grid10->header->inc[GMT_X],grid10->header->ny,grid10->header->inc[GMT_Y]);
    if ((num_rng[9] != round(grid10->header->nx*grid10->header->inc[GMT_X])) || (num_azi[9] != round(grid10->header->ny * grid10->header->inc[GMT_Y]))) {
      die("\nPRM file and GMT grid don't match!\n",argv[20]);
    }
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[20], grid10) == NULL) return EXIT_FAILURE;
    ...
    if ((grid1 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node1 = 0; node1 < grid1->header->size; node1++) grid1->data[node1] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid2 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node2 = 0; node2 < grid2->header->size; node2++) grid2->data[node2] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid3 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node3 = 0; node3 < grid3->header->size; node3++) grid3->data[node3] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid4 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node4 = 0; node4 < grid4->header->size; node4++) grid4->data[node4] = NAN;
   ...
   /* subswath 5 */
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid5 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node5 = 0; node5 < grid5->header->size; node5++) grid5->data[node5] = NAN;
  ...
  /* subswath 6 */
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid6 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
  ...
  /* subswath 7 */
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid7 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node7 = 0; node7 < grid7->header->size; node7++) grid7->data[node7] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid8 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node8 = 0; node8 < grid8->header->size; node8++) grid8->data[node8] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid9 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */
    for (node9 = 0; node9 < grid9->header->size; node9++) grid9->data[node9] = NAN;
    ...
    fprintf(stderr, "wesn = %f %f %f %f \n", wesn[GMT_XLO], wesn[GMT_YLO], wesn[GMT_XHI], wesn[GMT_YHI]);
    if ((grid10 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
    /* fill the empty grid with nan */    for (node10 = 0; node10 < grid10->header->size; node10++) grid10->data[node10] = NAN;
  }
  ...
  if (dir == 1) {
    ...
    if ((grid11 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");

    /* fill the empty grid with nan */
    for (node11 = 0; node11 < grid11->header->size; node11++) grid11->data[node11] = NAN;
    ...
    for (range = ra[0][0]; range < range_boundary1; range ++) { 
      for (azimuth = ra[0][2]; azimuth < azimuth_boundary1; azimuth++) {
        ...
        row = grid1->header->ny-round((azimuth*dec-clock_start_rel[0]*vel[0])/(vel[0]/prf[0])/grid1->header->inc[GMT_Y]);
    ...
    for (range = range_boundary1; range < range_boundary2; range ++) {
      for (azimuth = ra[1][2]; azimuth < azimuth_boundary2; azimuth++) {
        ...
        row = grid2->header->ny-round((azimuth*dec-clock_start_rel[1]*vel[1])/(vel[1]/prf[1])/grid2->header->inc[GMT_Y]);
    ...
    for (range = range_boundary2; range < range_boundary3; range ++) {
      for (azimuth = ra[2][2]; azimuth < azimuth_boundary3; azimuth++) {
        ...
        row = grid3->header->ny-round((azimuth*dec-clock_start_rel[2]*vel[2])/(vel[2]/prf[2])/grid3->header->inc[GMT_Y]);
    ...
    for (range = range_boundary3; range < range_boundary4; range ++) {
      for (azimuth = ra[3][2]; azimuth < azimuth_boundary4; azimuth++) {
        ...
        row = grid4->header->ny-round((azimuth*dec-clock_start_rel[3]*vel[3])/(vel[3]/prf[3])/grid4->header->inc[GMT_Y]);
    ...
    for (range = range_boundary4; range < range_boundary5; range ++) {
      for (azimuth = ra[4][2]; azimuth < azimuth_boundary5; azimuth++) {
        ...
        row = grid5->header->ny-round((azimuth*dec-clock_start_rel[4]*vel[4])/(vel[4]/prf[4])/grid5->header->inc[GMT_Y]);
    ...
    for (range = ra[5][0]; range < range_boundary1; range ++) {
      for (azimuth = azimuth_boundary1; azimuth < ra[5][3]; azimuth++) {
        ...
        row = grid6->header->ny-round((azimuth*dec-clock_start_rel[5]*vel[5])/(vel[5]/prf[5])/grid6->header->inc[GMT_Y]);
    ...
    for (range = range_boundary1; range < range_boundary2; range ++) {
      for (azimuth = azimuth_boundary2; azimuth < ra[6][3]; azimuth++) {
        ...
        row = grid7->header->ny-round((azimuth*dec-clock_start_rel[6]*vel[6])/(vel[6]/prf[6])/grid7->header->inc[GMT_Y]);
    ...
    for (range = range_boundary2; range < range_boundary3; range ++) {
      for (azimuth = azimuth_boundary3; azimuth < ra[7][3]; azimuth++) {
        ...
        row = grid8->header->ny-round((azimuth*dec-clock_start_rel[7]*vel[7])/(vel[7]/prf[7])/grid8->header->inc[GMT_Y]);
    ...
    for (range = range_boundary3; range < range_boundary4; range ++) {
      for (azimuth = azimuth_boundary4; azimuth < ra[8][3]; azimuth++) {
        ...
        row = grid9->header->ny-round((azimuth*dec-clock_start_rel[8]*vel[8])/(vel[8]/prf[8])/grid9->header->inc[GMT_Y]);
    ...
    for (range = range_boundary4; range < range_boundary5; range ++) {
      for (azimuth = azimuth_boundary5; azimuth < ra[9][3]; azimuth++) {
        ...
        row = grid10->header->ny-round((azimuth*dec-clock_start_rel[9]*vel[9])/(vel[9]/prf[9])/grid10->header->inc[GMT_Y]);
    ...
    /* output and clean up */
    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[21], grid11)) return EXIT_FAILURE;

  }
  else if (dir == 0) {
    fprintf(stderr, "divide the mosaic into 5 subswath...\n");
    /* open the mosaic grid */
    if ((grid11 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[21], NULL)) == NULL) return EXIT_FAILURE;
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[21], grid11) == NULL) return EXIT_FAILURE;
    ...
    for (col = 0; col < grid1->header->nx; col++) {
      for (row = 0; row < grid1->header->ny; row++) {
        ...
        azimuth = round((clock_start_rel[0]*vel[0]+(grid1->header->ny-row)*grid1->header->inc[GMT_Y]*vel[0]/prf[0])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    for (col = 0; col < grid2->header->nx; col++) {
      for (row = 0; row < grid2->header->ny; row++) {
        ...
        azimuth = round((clock_start_rel[1]*vel[1]+(grid2->header->ny-row)*grid2->header->inc[GMT_Y]*vel[1]/prf[1])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
   ...
   /* subswath 3 */
    for (col = 0; col < grid3->header->nx; col++) {
      for (row = 0; row < grid3->header->ny; row++) {
        ...
        azimuth = round((clock_start_rel[2]*vel[2]+(grid3->header->ny-row)*grid3->header->inc[GMT_Y]*vel[2]/prf[2])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    for (col = 0; col < grid4->header->nx; col++) {
      for (row = 0; row < grid4->header->ny; row++) {
        ...
        azimuth = round((clock_start_rel[3]*vel[3]+(grid4->header->ny-row)*grid4->header->inc[GMT_Y]*vel[3]/prf[3])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    for (col = 0; col < grid5->header->nx; col++) {
      for (row = 0; row < grid5->header->ny; row++) {
        ...
        azimuth = round((clock_start_rel[4]*vel[4]+(grid5->header->ny-row)*grid5->header->inc[GMT_Y]*vel[4]/prf[4])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    /* subswath 6 */
    for (col = 0; col < grid6->header->nx; col++) {
      for (row = 0; row < grid6->header->ny; row++) {
        node6 = GMT_Get_Index(API,grid6->header,row,col);
        range = round((near_range_rel[5]+col*grid6->header->inc[GMT_X]*dr)/dec);
        azimuth = round((clock_start_rel[5]*vel[5]+(grid6->header->ny-row)*grid6->header->inc[GMT_Y]*vel[5]/prf[5])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    /* subswath 7 */
    for (col = 0; col < grid7->header->nx; col++) {
      for (row = 0; row < grid7->header->ny; row++) {
        node7 = GMT_Get_Index(API,grid7->header,row,col);
        range = round((near_range_rel[6]+col*grid7->header->inc[GMT_X]*dr)/dec);
        azimuth = round((clock_start_rel[6]*vel[6]+(grid7->header->ny-row)*grid7->header->inc[GMT_Y]*vel[6]/prf[6])/dec);
	if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
        node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    /* subswath 8 */
    for (col = 0; col < grid8->header->nx; col++) {
      for (row = 0; row < grid8->header->ny; row++) {
        node8 = GMT_Get_Index(API,grid8->header,row,col);
        range = round((near_range_rel[7]+col*grid8->header->inc[GMT_X]*dr)/dec);
        azimuth = round((clock_start_rel[7]*vel[7]+(grid8->header->ny-row)*grid8->header->inc[GMT_Y]*vel[7]/prf[7])/dec);
        if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    /* subswath 9 */
    for (col = 0; col < grid9->header->nx; col++) {
      for (row = 0; row < grid9->header->ny; row++) {
        node9 = GMT_Get_Index(API,grid9->header,row,col);
        range = round((near_range_rel[8]+col*grid9->header->inc[GMT_X]*dr)/dec);
        azimuth = round((clock_start_rel[8]*vel[8]+(grid9->header->ny-row)*grid9->header->inc[GMT_Y]*vel[8]/prf[8])/dec);
	if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    /* subswath 10 */
    for (col = 0; col < grid10->header->nx; col++) {
      for (row = 0; row < grid10->header->ny; row++) {
        node10 = GMT_Get_Index(API,grid10->header,row,col);
        range = round((near_range_rel[9]+col*grid10->header->inc[GMT_X]*dr)/dec);
        azimuth = round((clock_start_rel[9]*vel[9]+(grid10->header->ny-row)*grid10->header->inc[GMT_Y]*vel[9]/prf[9])/dec);
	if (range >= 0 && range <= grid11->header->nx && azimuth >= 0 && azimuth <= grid11->header->ny) {
          node11 = GMT_Get_Index(API,grid11->header,azimuth,range);
    ...
    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[2], grid1)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[4], grid2)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[6], grid3)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[8], grid4)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[10], grid5)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[12], grid6)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[14], grid7)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[16], grid8)) return EXIT_FAILURE;
    
    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[18], grid9)) return EXIT_FAILURE;

    if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[20], grid10)) return EXIT_FAILURE;
