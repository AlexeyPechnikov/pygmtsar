 ...
 * SC_height_end parameters               * 07/27/10    merged modifications by
 * Jeff B to handle ALOSE ERSDAC format use ALOS_format to distinguish
 * *****************************************************************************/
...
void calc_height_velocity(struct ALOS_ORB *orb, struct PRM *prm, double t1, double t2, double *height, double *re2, double *vg,
	...
	if (ALOS_format == 0)
		nt = (prm->nrows - prm->num_valid_az) / 100.0;
	if (ALOS_format == 1)
		nt = 31;
