 ...
 * Date:                                                                        *
 * add fs0, read fs from PRM and check consistence, M.Wei, 5/2/08               *
 * 
...
FILE	*prmfile;
	...
	switch(r[0].SC_identity)
		...
		default: fprintf(stderr,"Arghhh...unrecognized satellite\n");
		}
...
/*	int i;		*/
	...
	for (ii=1; ii<nfiles; ii++){
		...
		for (k = -ns2; k<ns + ns2; k++){
			...
			if (b1 < 0.0 || ds1 < b1) endpoint_distance(k, ds1, xs, ys, zs, &b1, &x11, &y11, &z11, &m1); 

			ds2 = find_distance(xs, ys, zs, x22, y22, z22);
			if (b2 < 0.0 || ds2 < b2) endpoint_distance(k, ds2, xs, ys, zs, &b2, &x12, &y12, &z12, &m2); 

			ds3 = find_distance(xs, ys, zs, x23, y23, z23);
			if (b3 < 0.0 || ds3 < b3) endpoint_distance(k, ds3, xs, ys, zs, &b3, &x13, &y13, &z13, &m3); 
		...
		if (x13 == -99999.0) die("x13 not initialized","");
		...
		if(b1 == 0. && b2 == 0.) r[ii].baseline_center = 0.;
