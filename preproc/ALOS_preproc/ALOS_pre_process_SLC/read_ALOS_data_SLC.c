...
long read_ALOS_data_SLC (FILE *imagefile, FILE *outfile, struct PRM *prm, long *byte_offset) {
	...
	/* allocate memory */
	if ((rdata = (float *) malloc(record_length0)) == NULL) die("couldn't allocate memory for input indata.\n","");
	...
	while ( (fread((void *) &sdr,sizeof(struct sardata_info), 1, imagefile)) == 1 ) {
		...
		if(n == 2) start_sdr_rec_len = sdr.record_length;
		...
		data_length = record_length1/4;
          	...
          	if (fread ((float *) rdata, 4, data_length, imagefile) != data_length ) break;
		...
		/* write line header to output data  */
          	/*fwrite((void *) &sdr, line_prefix_size, 1, outfile);*/
                ...
                if(swap)swap32(rdata,rdata_swap,data_length);
                for (jj=0;jj<data_length;jj++){
                  sgn=1.;
                  //if(jj%2 != 0) sgn = -1.;  experiment to switch sign of imaginary component
                  if(swap) {
...
int	k;
	...
	/* NULL_DATA = 15; znew randonly is 0 or 1			      */
       	if (shift > 0) {					
         	for (k = 0; k < ishift; k++) shift_data[k] = NULL_DATA;
	...
	/* if data is shifted, fill in with data vlues of NULL_DATA at end */
	  if ( shift < 0) {
            	...
            	for (k = data_length - ishift - line_suffix_size; k < record_length1; k++ ) shift_data[k] = NULL_DATA;
          	}
