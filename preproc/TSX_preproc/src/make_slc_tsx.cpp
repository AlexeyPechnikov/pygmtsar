...
main(int argc, char *argv[])
{
	...
	if (vm.count("version")) {
		std::cout << "make_slc_tsx " << PACKAGE_VERSION << "\n"
	...
	TSX tsx = TSX(filename);
	std::cout << "Extracting Image" << std::endl;
	SingleBandImage<std::complex<short> > *image = tsx.extractSlc_short_Image(outputImage);
	if (image == NULL)
	{
		return 1;
	}

	GMTSAR::writeEphemeris(outputOrbit,tsx.getOrbit(),tsx.getScene());
	GMTSAR::writeResourceFile(outputRSC,tsx.getScene(),tsx.getInstrument(),tsx.getOrbit(),image);
