...
class SBAS:
    ...
    def get_dem(self, subswath=None, geoloc=False, buffer_degrees=0.02):
        ...
        dem = xr.open_dataset(self.dem_filename, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def get_landmask(self, geoloc=False, inverse_geocode=False, buffer_degrees=0.02):
        ...
        # open DEM file and find the elevation variable
        # because sometimes grid includes 'crs' or other variables
        landmask = xr.open_dataset(self.landmask_filename, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def intf_ra2ll(self, subswath=None, grids=None, debug=False):
        ...
        matrix_ra2ll = xr.open_dataarray(intf_ra2ll_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
        matrix_ll2ra = xr.open_dataarray(intf_ll2ra_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def intf_ra2ll_matrix(self, subswath, intf_grids, debug=False):
        ...
        trans_ra2ll = xr.open_dataarray(trans_ra2ll_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def get_topo_ra(self, subswath=None):
        ...
        for subswath in subswaths:
            ...
            topo = xr.open_dataarray(topo_ra_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
            # flip vertically for GMTSAR compatibility reasons
    ...
    def sat_look_parallel(self, n_jobs=-1, interactive=False, debug=False):
        ...
        for subswath in subswaths:
            ...
            dem = self.get_dem(subswath, geoloc=True)
            grid_ll = xr.open_dataarray(intf_ra2ll_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def get_sat_look(self, subswath=None):
        ...
        for subswath in subswaths:
            ...
            sat_look = xr.open_dataset(sat_look_file, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def unwrap(self, subswath, pair, threshold=None, conf=None, func=None, mask=None, conncomp=False,
        ...
        phase = xr.open_dataarray(phase_filename, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
        corr = xr.open_dataarray(corr_filename, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
        if mask is not None:
    ...
    def open_grids(self, pairs, name, geocode=False, inverse_geocode=False,  mask=None, func=None,
        ...
        for subswath in subswaths:
            ...
            if pairs is None:
                ...
                da = xr.open_dataarray(filename, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize)
    ...
    def sbas_parallel(self, pairs, mask=None, detrended=True, n_jobs=-1):
        ...
        das = [xr.open_dataarray(f, engine=self.netcdf_engine, chunks=2*self.netcdf_chunksize) for f in filenames]
        if xr.__version__ == '0.19.0':
            # for Google Colab
            das = xr.merge(das)
        else:
