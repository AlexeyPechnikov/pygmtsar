...
#         return out.rename('tidal')
    ...
    def get_tidal(self):
        return self.open_grid('tidal', chunksize={'date': 1, 'y': self.chunksize, 'x': self.chunksize})
    ...
    def compute_tidal(self, dates=None, coarsen=32, interactive=False):
        ...
        for idx, date in enumerate(dates):
            ...
            out = np.fromstring(stdout_data, dtype=np.float32, sep=' ')
        ...
        if interactive:
            return ds
        #.rename({'y': 'a', 'x': 'r'})
        self.save_grid(ds, 'tidal', 'Solid Earth Tides Saving')
