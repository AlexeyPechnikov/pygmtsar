...
class SBAS_trans_inv(SBAS_trans):
    ...
    def get_trans_inv(self, subswath=None, chunksize=None):
        ...
        get_trans_inv()
        """
        return self.open_grid('trans_inv', subswath=subswath, chunksize=chunksize)

    def trans_inv(self, subswath, coarsen, method='linear', chunksize=None, interactive=False):
        """
    ...
    #         return np.asarray(output)
        ...
        trans_dat = self.get_trans(subswath)
        ...
        return self.save_grid(trans_inv.rename({'y': 'a', 'x': 'r'}),
                              'trans_inv', subswath, f'Radar Inverse Transform Computing sw{subswath}', chunksize)
