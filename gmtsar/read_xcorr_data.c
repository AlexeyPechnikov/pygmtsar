...
void read_complex_short2(FILE *f, struct FCOMPLEX *d, int iy, int npy, int nx, short *tmp)
{
	...
	for (i=0; i<npy; i++){
		...
		for (j=0; j<nx; j++) {
			...
			//d[i*nx+j].r = sqrt(((float)tmp[2*j])*((float)tmp[2*j])+((float)tmp[2*j+1])*((float)tmp[2*j+1]));
			//d[i*nx+j].i = 0.0;
			}
		}
}
...
float   *tmp2_m,*tmp2_s;
        ...
        if (xc->format == 0) read_complex_short2(xc->data1, xc->d1, iy, xc->npy, xc->m_nx, tmp_m);
        if (xc->format == 1) read_real_float2(xc->data1, xc->d1, iy, xc->npy, xc->m_nx, tmp2_m);
        if (xc->format == 2) read_real_float_grid(xc->D1, xc->d1, iy, xc->npy, xc->m_nx, xc->m_ny);
        ...
        if (xc->format == 0) read_complex_short2(xc->data2, xc->d2, iy, xc->npy, xc->s_nx, tmp_s);
        if (xc->format == 1) read_real_float2(xc->data2, xc->d2, iy, xc->npy, xc->s_nx, tmp2_s);
        if (xc->format == 2) read_real_float_grid(xc->D2, xc->d2, iy, xc->npy, xc->s_nx, xc->s_ny);
