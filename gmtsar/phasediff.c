...
void calc_drho(int xdim, double *range, double *topo, double avet, double re,
		double height, double B, double alpha, double Bx, double *drho)
{
int k;
...
void read_optional_args(void *API, int argc, char **argv, struct PRM *tp, int *topoflag, struct PRM *mp, int *modelflag)
{
	...
	for (i=3; i<argc; i++) {
		if (strcmp(argv[i],"-topo") == 0) {
			...
			if ((T = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, tp->input_file, NULL)) == NULL) die("cannot open topofile",tp->input_file);
		...
		if (strcmp(argv[i],"-model") == 0) {
			...
			if ((M = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, mp->input_file, NULL)) == NULL) die("cannot open model file",mp->input_file);
...
int main (int argc, char **argv)
	...
	if (argc > 3) read_optional_args(API, argc, argv, &tp, &topoflag, &mp, &modelflag);
	...
	wesn[GMT_XLO] = 0.0;	wesn[GMT_XHI] = inc[GMT_X] * xdim;
	wesn[GMT_YLO] = 0.0;	wesn[GMT_YHI] = inc[GMT_Y] * ydim;
	if ((RE = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, \
		GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
	if (GMT_Set_Comment (API, GMT_IS_GRID, GMT_COMMENT_IS_REMARK, "real", RE)) return EXIT_FAILURE;
	if ((IM = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, \
		GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");
	if (GMT_Set_Comment (API, GMT_IS_GRID, GMT_COMMENT_IS_REMARK, "imag", RE)) return EXIT_FAILURE;
	...
	if (topoflag) {
		if ((T = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, tp.input_file, NULL)) == NULL) return EXIT_FAILURE;
		if (xdim % T->header->nx != 0 || ydim % T->header->ny != 0)
			die("The dimension SLC must be multiplication factor of the topo_ra", tp.input_file); 
		if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, tp.input_file, T) == NULL) return EXIT_FAILURE;
		rdumt = floor (T->header->z_max + 1.0);
	   			
		if(verbose) fprintf(stderr,"\n%s %s %d %d\n", T->header->title, tp.input_file, T->header->nx, T->header->ny);
		if(verbose) fprintf(stderr,"\n%f %f %f %f %f\n", T->header->wesn[GMT_XLO], T->header->wesn[GMT_YLO], T->header->inc[GMT_X], T->header->inc[GMT_Y], rdumt); 
	...
	if (modelflag) {
		if ((M = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, mp.input_file, NULL)) == NULL) return EXIT_FAILURE;
		...
		if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, mp.input_file, M) == NULL) return EXIT_FAILURE;
...
if (p2.SC_identity == 100) {
    ...
    p2.baseline_start = sqrt(pow((p1.ashift+p1.sub_int_a - p2.ashift-p2.sub_int_a)*p2.vel/p2.prf,2)+pow(tmpB,2));
    p2.alpha_start = acos(tmpB*sin(alpha*PI/180.0)/p2.baseline_start)/PI*180.0;

    tmpB = p2.baseline_end;
    alpha = p2.alpha_end;
    p2.baseline_end = sqrt(pow((p1.ashift+p1.sub_int_a - p2.ashift-p2.sub_int_a)*p2.vel/p2.prf,2)+pow(tmpB,2));
    p2.alpha_end = acos(tmpB*sin(alpha*PI/180.0)/p2.baseline_end)/PI*180.0;

    tmpB = p2.baseline_center;
    alpha = p2.alpha_center;
    p2.baseline_center = sqrt(pow((p1.ashift+p1.sub_int_a - p2.ashift-p2.sub_int_a)*p2.vel/p2.prf,2)+pow(tmpB,2));
    p2.alpha_center = acos(tmpB*sin(alpha*PI/180.0)/p2.baseline_center)/PI*180.0;
}
*/
