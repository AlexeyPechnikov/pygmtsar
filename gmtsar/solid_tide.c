...
//call zero_vec8(xcorsta)
    xcorsta[1] = 0;xcorsta[2] = 0;xcorsta[3] = 0;xcorsta[0] = 0;
...
//*** units of mm
    double datdi[45]	/* was [9][5] */ = { 0.,0.,0.,1.,0.,.47,.23,
...
//*** columns are s,h,p,N',ps, dR(ip),dT(ip),dR(op),dT(op)
    ...
    for (j=1;j<=5;j++){
        thetaf=(datdi[j*9-9]*s+datdi[j*9-8]*h+datdi[j*9-7]*p+datdi[j*9-6]*zns+datdi[j*9-5]*ps)*deg2rad;
        dr=datdi[j*9-4]*(3.0*sinphi*sinphi-1.0)/2.0*cos(thetaf)+datdi[j*9-2]*(3.0*sinphi*sinphi-1.0)/2.0*sin(thetaf);
...
//*** units of mm
    ...
    double datdi[279]	/* was [9][31] */ = { -3.,0.,2.,0.,0.,
...
//*** reduce angles to between 0 and 360
    ...
    for (j=1;j<=31;j++){
        thetaf=(tau+datdi[j*9-9]*s+datdi[j*9-8]*h+datdi[j*9-7]*p+datdi[j*9-6]*zns+datdi[j*9-5]*ps)*deg2rad;
        dr=datdi[j*9-4]*2.0*sinphi*cosphi*sin(thetaf+zla)+datdi[j*9-3]*2.0*sinphi*cosphi*cos(thetaf+zla);
        dn=datdi[j*9-2]*(cosphi*cosphi-sinphi*sinphi)*sin(thetaf+zla)+datdi[j*9-1]*(cosphi*cosphi-sinphi*sinphi)*cos(thetaf+zla);
...
//*** for the diurnal band
    ...
    desun=l1*sinphi*(cosphi*cosphi-sinphi*sinphi)*fac2sun*xsun[3]*(xsun[1]*sinla-xsun[2]*cosla)/rsun/rsun;
    demon=l1*sinphi*(cosphi*cosphi-sinphi*sinphi)*fac2mon*xmon[3]*(xmon[1]*sinla-xmon[2]*cosla)/rmon/rmon;
    de=3.0*(desun+demon);
    dn=3.0*(dnsun+dnmon);
...
//*** for the semi-diurnal band
    ...
    dnsun=-l1/2.0*sinphi*cosphi*fac2sun*((xsun[1]*xsun[1]-xsun[2]*xsun[2])*costwola+2.0*xsun[1]*xsun[2]*sintwola)/rsun/rsun;
    dnmon=-l1/2.0*sinphi*cosphi*fac2mon*((xmon[1]*xmon[1]-xmon[2]*xmon[2])*costwola+2.0*xmon[1]*xmon[2]*sintwola)/rmon/rmon;
    desun=-l1/2.0*sinphi*sinphi*cosphi*fac2sun*((xsun[1]*xsun[1]-xsun[2]*xsun[2])*sintwola-2.0*xsun[1]*xsun[2]*costwola)/rsun/rsun;
    demon=-l1/2.0*sinphi*sinphi*cosphi*fac2mon*((xmon[1]*xmon[1]-xmon[2]*xmon[2])*sintwola-2.0*xmon[1]*xmon[2]*costwola)/rmon/rmon;
    de=3.0*(desun+demon);
    dn=3.0*(dnsun+dnmon);
...
//data dhi/-0.0022d0/,dli/-0.0007d0/
    ...
    double dhi,dli,rsta,sinphi,cosphi,sinla,cosla,costwola,sintwola,rmon,rsun,drsun,drmon,dnsun,dnmon,desun,demon,dr,dn,de;
    ...
    rmon=enorm8(xmon);
    rsun=enorm8(xsun);
    drsun=-3.0/4.0*dhi*cosphi*cosphi*fac2sun*((xsun[1]*xsun[1]-xsun[2]*xsun[2])*sintwola-2.0*xsun[1]*xsun[2]*costwola)/rsun/rsun;
    drmon=-3.0/4.0*dhi*cosphi*cosphi*fac2mon*((xmon[1]*xmon[1]-xmon[2]*xmon[2])*sintwola-2.0*xmon[1]*xmon[2]*costwola)/rmon/rmon;
    dnsun=1.50*dli*sinphi*cosphi*fac2sun*((xsun[1]*xsun[1]-xsun[2]*xsun[2])*sintwola-2.0*xsun[1]*xsun[2]*costwola)/rsun/rsun;
    dnmon=1.50*dli*sinphi*cosphi*fac2mon*((xmon[1]*xmon[1]-xmon[2]*xmon[2])*sintwola-2.0*xmon[1]*xmon[2]*costwola)/rmon/rmon;
    desun=-3.0/2.0*dli*cosphi*fac2sun*((xsun[1]*xsun[1]-xsun[2]*xsun[2])*costwola+2.0*xsun[1]*xsun[2]*sintwola)/rsun/rsun;
    demon=-3.0/2.0*dli*cosphi*fac2mon*((xmon[1]*xmon[1]-xmon[2]*xmon[2])*costwola+2.0*xmon[1]*xmon[2]*sintwola)/rmon/rmon;
    dr=drsun+drmon;
    dn=dnsun+dnmon;
    de=desun+demon;
...
//data dhi/-0.0025d0/,dli/-0.0007d0/
    double dhi,dli,rsta,sinphi,cosphi,cos2phi,sinla,cosla,rmon,rsun,drsun,drmon,dnsun,dnmon,desun,demon,dr,dn,de;
...
//*** use TT for lunar ephemerides
    ...
    double tsecgps,tsectt,fmjdtt,tjdtt,t,el0,el,elp,f,d,selond,q,selatd,rse,oblir,sselat,cselat,sselon,cselon,t1,t2,t3;
    double ghar;
...
//*** longitude w.r.t. equinox and ecliptic of year 2000
    ...
    +22640.0/3600.0*sin((el        )/rad)
    +  769.0/3600.0*sin((el+el     )/rad)
    - 4586.0/3600.0*sin((el-d-d    )/rad)
    + 2370.0/3600.0*sin((d+d       )/rad)
    -  668.0/3600.0*sin((elp       )/rad)
    -  412.0/3600.0*sin((f+f       )/rad)
    -  212.0/3600.0*sin((el+el-d-d )/rad)
    -  206.0/3600.0*sin((el+elp-d-d)/rad)
    +  192.0/3600.0*sin((el+d+d    )/rad)
    -  165.0/3600.0*sin((elp-d-d   )/rad)
    +  148.0/3600.0*sin((el-elp    )/rad)
    -  125.0/3600.0*sin((d         )/rad)
    -  110.0/3600.0*sin((el+elp    )/rad)
...
//*** latitude w.r.t. equinox and ecliptic of year 2000
    ...
    selatd=                                         //!*** eq 3.49, p.72
    +18520.0/3600.0*sin((f+selond-el0+q)/rad)
    -  526.0/3600.0*sin((f-d-d     )/rad)
    +   44.0/3600.0*sin((el+f-d-d  )/rad)
    -   31.0/3600.0*sin((-el+f-d-d )/rad)
    -   25.0/3600.0*sin((-el-el+f  )/rad)
    -   23.0/3600.0*sin((elp+f-d-d )/rad)
    +   21.0/3600.0*sin((-el+f     )/rad)
    +   11.0/3600.0*sin((-elp+f-d-d)/rad);

//*** distance from Earth center to Moon (m)

    rse= 385000.0*1000.0                          //!*** eq 3.50, p.72
    -  20905.0*1000.0*cos((el        )/rad)
    -   3699.0*1000.0*cos((d+d-el    )/rad)
    -   2956.0*1000.0*cos((d+d       )/rad)
    -    570.0*1000.0*cos((el+el     )/rad)
    +    246.0*1000.0*cos((el+el-d-d )/rad)
    -    205.0*1000.0*cos((elp-d-d   )/rad)
    -    171.0*1000.0*cos((el+d+d    )/rad)
    -    152.0*1000.0*cos((el+elp-d-d)/rad);
...
int civmjd(double iyr,double imo,double idy,double ihr,double imn,double sec,double *mjd,double *fmjd) {
    ...
    double m,it1,it2;
    int y;
    if(iyr < 1900) die("year should not be smaller than 1900",""); //stop 34588
