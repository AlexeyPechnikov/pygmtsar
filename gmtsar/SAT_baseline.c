 ...
 * Date   :  06/07/2007                                                         *
 * Modifications:                                                               *
 * 29/26/2018 by Xiaohua Xu                                                     *
 *           Adding the along track component for Baseline called B_offset      *
 *           This further ensures phase closure                                 *
 ********************************************************************************/
...
void baseline(struct PRM *r, struct SAT_ORB *orb, int nfiles, int input_flag, char **filename,double fs0) {
    ...
    for (k = -ns2; k<ns + ns2; k++){
        ...
        if (b1 < 0.0 || ds < b1) endpoint_distance(k, ds, xs, ys, zs, &b1, &x21, &y21, &z21, &m1);

        ds = find_dist(xs, ys, zs, x12, y12, z12);
        if (b2 < 0.0 || ds < b2) endpoint_distance(k, ds, xs, ys, zs, &b2, &x22, &y22, &z22, &m2);

        ds = find_dist(xs, ys, zs, x13, y13, z13);
        if (b3 < 0.0 || ds < b3) endpoint_distance(k, ds, xs, ys, zs, &b3, &x23, &y23, &z23, &m3);
    }
...
*/
    ...
    /* shouldn't happen ..					*/
    if (x21 == -99999.0 ) die("x11 not initialized","");
    if (x22 == -99999.0) die("x12 not initialized","");
