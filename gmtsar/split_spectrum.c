...
int split2(int argc2, char **argv2) {
    ...
    if ((SLCH1 = fopen("SLCH1","wb")) == NULL) die("Can't open ","SLCH1");
    if ((SLCH2 = fopen("SLCH2","wb")) == NULL) die("Can't open ","SLCH2");
    if ((SLCL1 = fopen("SLCL1","wb")) == NULL) die("Can't open ","SLCL1");
    if ((SLCL2 = fopen("SLCL2","wb")) == NULL) die("Can't open ","SLCL2");
...
int cos_window(double fc,double fb,double fs,int N, double *filter) {
    ...
    // modify the section below to tune the band filter parameters , 2*flat_nb+2*cos_nb = 1
    flat_nb = (int)round(nb/4);
    cos_nb = (int)round(nb/4);
    
    
    for (i=nc-flat_nb-1;i<nc+flat_nb;i++) filter[i] = 1;
    for (i=nc-flat_nb-cos_nb-1;i<nc-flat_nb-1;i++) filter[i] = 0.5-cos(PI*(i-(nc-flat_nb-cos_nb-1)+1)/cos_nb)/2.0;
    for (i=nc+flat_nb;i<nc+flat_nb+cos_nb;i++) filter[i] = cos(PI*(i-(nc+flat_nb)+1)/cos_nb)/2.0+0.5;
...
void bandpass_filter(double bw, double bc, int n, int nfft, int ncshift, double alpha, fcomplex *filter) {
