...
int get_range (char *str, int *xl, int *xh, int *yl, int *yh){
    ...
    tmp_c[jj] = c;
    rr[kk] = atoi(tmp_c);
    *xl = rr[0]; *xh = rr[1]; *yl = rr[2]; *yh = rr[3];
...
int main (int argc, char **argv) {
    ...
    if (xl < 0 || xl > xh || xh > nr-1 || yl < 0 || yl > yh || yh > nl-1) die("wrong range ","");

    if ((SLC_in = fopen(pp.SLC_file,"rb")) == NULL) die("Can't open SLCfile",pp.SLC_file);

    strcpy(str,argv[2]);
    //str[strlen(str)-3] = '\0';
    strcat(str,".SLC");

    if ((SLC_out = fopen(str,"wb")) == NULL) die("Can't open SLCfile",str);
    ...
    strcpy(str,argv[2]);
    strcat(str,".PRM");
    if (( PRM_out = fopen(str,"w")) == NULL) die("can't open prm file",str);
