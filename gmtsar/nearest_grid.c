    ...
    for (i=0;i<ny;i++) {
        ...
        for (j=0;j<nx;j++) {
            ...
            if (isnan(m[i*nx+j]) == 0) {
                ...
                flag=0;
                    //if (rr >= 1) rr = ((int)sqrt((double)rr)-1)*((int)sqrt((double)rr)-1);
                ...
                if (rr >= 4 && recy > 0 && recx > 0) rr = (recx-1)*(recx-1)+(recy-1)*(recy-1)-1;
                    ...
                    //else if (idx[j] != 0 && bufr[j] >= 4 && bufx[j] > 0) rr = (bufx[j]-1)*(bufx[j]-1) + bufy[j]*bufy[j] -1;
                    //else if (idx[j] != 0 && bufr[j] >= 4 && bufx[j] == 0) rr = bufy[j]*bufy[j] -1;
...
//                                bufx[j] = abs(is[k]-i);
//                                bufy[j] = abs(js[k]-j);
//                                bufr[j] = bufx[j]*bufx[j]+bufy[j]*bufy[j];
                                break;
                            }
                        }
                    }
...
int create_grid(void *API, char *file, char *output, int radius) {
    ...
    if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, file, T) == NULL) return EXIT_FAILURE;
