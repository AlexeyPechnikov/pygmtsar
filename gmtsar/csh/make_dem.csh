...
# bytes swap
      if ($system =~ "sparc" || $system =~ "powerpc") then 
        gmt xyz2grd $demtmp -G$demtmpgrd"=bf" -I$pixel -R$range  -di-32768 -ZTLh $V 
      else if ($system =~ "i686" || $system =~ "i386") then
        gmt xyz2grd $demtmp -G$demtmpgrd"=bf" -I$pixel -R$range  -di-32768 -ZTLhw $V
      endif

# paste grd files together along longitude
      if (-e tmplon.grd) then
         gmt grdpaste $demtmpgrd"=bf" tmplon.grd"=bf" -Gtmplon.grd"=bf"
      else
...
# paste grd files together along latitude
   if (-e tmplat.grd) then
      gmt grdpaste tmplon.grd"=bf" tmplat.grd"=bf" -Gtmplat.grd"=bf"
      rm -f tmplon.grd
   else
...
gmt grdcut -R$bound tmplat.grd"=bf" -Gdem_ortho.grd
