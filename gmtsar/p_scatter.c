...
/* output either the scatter function or the average amplitude */
	           ...
	           if(sig > .1) scatter->data[ii*xdimm+jj] = ave/(2.*sig);
		}
		else {
	   ...
	   for(jj=0; jj<xdimm; jj++) {
		...
		test2 = (float)slc_rows[mm+1] * scatter->data[ii*xdimm+jj];
  		slc_rows[mm+1] = round(test2);
                //if(fabs(test1) > 32767. || fabs(test2) > 32767.) fprintf(stderr," %f %f test out of bounds \n",test1,test2);
	      }
	   }
...
int     i;
FILE    *inputfile;
        if ((inputfile=fopen(inputfilename,"rw")) == NULL) die("Can't open ",inputfilename);
