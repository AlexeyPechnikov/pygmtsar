...
int get_prm(struct PRM *p, char *filename)
{
	...
	if ((PRMfile = fopen(filename,"r")) == NULL) die("can't open file",filename);
...
while(fscanf(fh,"%s = %s \n",name,value) != EOF){
	...
	if (strcmp(name, "earth_radius") == 0) get_double(name,"earth_radius", value, &s->RE);
        ...
        if (strcmp(name, "polar_radius") == 0) get_double(name,"polar_radius", value, &s->rc);
	...
	if (strcmp(name, "SC_height_start") == 0) get_double(name,"SC_height_start", value, &s->ht_start);
	if (strcmp(name, "SC_height_end") == 0) get_double(name,"SC_height_end", value, &s->ht_end);
	...
	if (strcmp(name, "baseline_center") == 0) get_double(name,"baseline_center", value, &s->baseline_center);
...
void put_sio_struct(struct PRM prm, FILE *OUTFILE)
{
	...
	if (prm.stretch_a != NULL_DOUBLE) fprintf(OUTFILE,"stretch_r   		= %g \n",prm.stretch_r);
	if (prm.stretch_a != NULL_DOUBLE) fprintf(OUTFILE,"stretch_a   		= %g \n",prm.stretch_a);
	if (prm.a_stretch_r != NULL_DOUBLE) fprintf(OUTFILE,"a_stretch_r   	= %g \n",prm.a_stretch_r);
	if (prm.a_stretch_a != NULL_DOUBLE) fprintf(OUTFILE,"a_stretch_a   	= %g \n",prm.a_stretch_a);
	...
	if (prm.ht_start != NULL_DOUBLE) fprintf(OUTFILE,"SC_height_start	= %lf \n",prm.ht_start);
	if (prm.ht_end != NULL_DOUBLE) fprintf(OUTFILE,"SC_height_end	= %lf \n",prm.ht_end);
...
/*
	...
	if (prm.ashift != NULL_INT) fprintf(OUTFILE, "ashift                  = %d\n",prm.ashift);
*/
	if (prm.sub_int_r != NULL_DOUBLE) fprintf(OUTFILE, "sub_int_r               = %lf \n",prm.sub_int_r);
	if (prm.sub_int_a != NULL_DOUBLE) fprintf(OUTFILE, "sub_int_a               = %lf \n",prm.sub_int_a);
	if (prm.bpara != NULL_DOUBLE) fprintf(OUTFILE, "B_parallel              = %lf \n",prm.bpara);
	if (prm.bperp != NULL_DOUBLE) fprintf(OUTFILE, "B_perpendicular         = %lf \n",prm.bperp);
	if (prm.baseline_start != NULL_DOUBLE) fprintf(OUTFILE, "baseline_start          = %lf \n",prm.baseline_start);
	if (prm.baseline_center != NULL_DOUBLE) fprintf(OUTFILE, "baseline_center          = %lf \n",prm.baseline_center);
	if (prm.baseline_end != NULL_DOUBLE) fprintf(OUTFILE, "baseline_end            = %lf \n",prm.baseline_end);
