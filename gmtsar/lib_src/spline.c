...
/* February 11, 2010 converted from fortran routines spline.f providrd by Bob Parker  
   the conversion was first performed using f2c.  Then Rob Mellors made some
   changes to eliminate the f2c.h file so the code has no dependencies.
   David Sandwell tested the cod against the fortran version and the differences
   were al the level of the machine precision except when the interpolated point is
   outside the bounds of the unput data.  In this case the fortran code and C-code
   diverge.  THis is not yet understood. */
...
double	q1, qn, c;
	...
	*istart = 0;
	...
	for (i = 2; i <= i1; ++i) {
		s[i] = (u[i-1]/(x[i]-x[i-1])-u[i]*(1.0/(x[i]-x[i-1])+1.0/(x[i+1]-x[i]))+u[i+1]/(x[i+1]-x[i]))*6.0;
		}
